...
contract Strategy is BaseStrategy {
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        ...
        if (_amountNeeded > available) {
            uint256 toWithdraw = _amountNeeded.sub(available);

            uint256 maxWithdrawal = ICToken(cToken).balanceOfUnderlying(address(this);
            // If we're trying to withdraw too much
            if(toWithdraw > maxWithdrawal){
                toWithdraw = maxWithdrawal;
                ICToken(cToken).redeemUnderlying(maxWithdrawal);
            } else {
                // Normal withdraw
                ICToken(cToken).redeemUnderlying(toWithdraw);
            }
        }
